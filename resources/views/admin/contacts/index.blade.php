                ...
                <tbody class="divide-y divide-gray-100">
                    @foreach ($submissions as $submission)
                        <tr class="{{ $submission->is_read ? '' : 'bg-indigo-50' }}">
                            ...
                            <td class="px-6 py-4 text-sm text-gray-600">{{ $submission->name }}</td>
                            <td class="px-6 py-4 text-sm">{{ $submission->is_read ? 'Dibaca' : 'Baru' }}</td>
                            <td class="px-6 py-4 text-sm text-gray-600">{{ $submission->created_at->format('d F Y H:i') }}</td>
