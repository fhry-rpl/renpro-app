...
class GalleryController extends Controller
{
    ...
    public function store(StoreGalleryRequest $request)
    {
        ...
        if ($request->hasFile('cover_image')) {
            $data['cover_image'] = $request->file('cover_image')->store('galleries', 's3');
        }
        $gallery = Gallery::create($data);
        foreach ($request->file('images', []) as $image) {
            $gallery->images()->create(['image_path' => $image->store('galleries', 's3')]);
        }
    ...
    public function update(UpdateGalleryRequest $request, Gallery $gallery)
    {
        $data = $request->validated();
        if ($request->hasFile('cover_image')) {
            Storage::disk('s3')->delete($gallery->cover_image);
            $data['cover_image'] = $request->file('cover_image')->store('galleries', 's3');
        }
        $gallery->update($data);
        if ($request->hasFile('images')) {
            foreach ($gallery->images as $oldImage) {
                Storage::disk('s3')->delete($oldImage->image_path);
            }
